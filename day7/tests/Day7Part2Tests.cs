...
public class Day7Part2Tests
{
    ...
    [Test]
    public void TwoCrabsCostIsTriangularNumberOfTheDistanceBetweenThem()
    {
        FindCostToAlignCrabs(new int[] {7, 8}).Should().Be(1);
        FindCostToAlignCrabs(new int[] {7, 9}).Should().Be(3);
        FindCostToAlignCrabs(new int[] {7, 10}).Should().Be(6);
        FindCostToAlignCrabs(new int[] {7, 11}).Should().Be(10);
        FindCostToAlignCrabs(new int[] {9, 7}).Should().Be(3);
    }
    ...
    {
        FindCostToAlignCrabs(new int[] {16,1,2,0,4,2,7,1,2,14}).Should().Be(168);
    }
    ...
    public void GoldenInput()
    {
        ...
        FindCostToAlignCrabs(crabs).Should().Be(93214037);
    }
    ...
    private int FindCostToAlignCrabs(int[] crabs)
    {
        ...
        {
            var distance = crabs.Max() - crabs.Min();
            return NthTriangularNumber(distance);
        }
        else if(crabs.Length >= 4)
        {
            var costs = new List<int>();
            var targetPositionRange = Enumerable.Range(crabs.Min(), crabs.Max());
            foreach(var targetPosition in targetPositionRange)
            {
                var cost = 0;
                foreach (var position in crabs)
                {
                    var distance = Math.Abs(position - targetPosition);
                    cost += NthTriangularNumber(distance);
                }
                costs.Add(cost);
            }
            return costs.Min();
    ...
    private static int NthTriangularNumber(int nthNumber)
    {
